...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -ldflags '-w -s' -o /go/bin/gateway gateway/gateway.go
...
COPY --from=builder /go/bin/gateway /go/bin/main
